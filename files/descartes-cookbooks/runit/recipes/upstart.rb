...
cookbook_file "/etc/init/descartes-runsvdir.conf" do
  owner "root"
  group "root"
  mode "0644"
  source "descartes-runsvdir.conf"
end

# Keep on trying till the job is found :(
execute "initctl status descartes-runsvdir" do
  retries 30
end
...
execute "initctl start descartes-runsvdir" do
  only_if "initctl status descartes-runsvdir | grep stop"
  retries 30
end
